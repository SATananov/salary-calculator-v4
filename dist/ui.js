...
export function getNumberValue(selector) {
...
export function getNewLocationData() {
    ...
    const typeSelect = getElement(selectors.locationType);
...
export function clearNewLocationForm() {
...
export function renderLocations(locations, onRemove) {
    const grid = getElement(selectors.locationsGrid);
...
export function populateLocationDropdowns(locations) {
    const dealerSelect = document.querySelector(selectors.dealerLocation);
    const filterSelect = document.querySelector(selectors.filterLocation);
    const options = locations.map(loc => `<option value="${loc.id}">${escapeHtml(loc.name)} (${escapeHtml(loc.city)})</option>`).join('');
    if (dealerSelect) {
...
export function getNewDealerData() {
    const name = getInputValue(selectors.dealerName);
    const locationSelect = getElement(selectors.dealerLocation);
...
export function clearNewDealerForm() {
...
function saveCurrentInputValues(dealers) {
    const savedValues = new Map();
    for (const dealer of dealers) {
        const salaryInput = document.querySelector(`#salary_${dealer.id}`);
        const turnoverInput = document.querySelector(`#turnover_${dealer.id}`);
        const vouchersInput = document.querySelector(`#vouchers_${dealer.id}`);
...
function restoreInputValues(savedValues) {
    savedValues.forEach((values, dealerId) => {
        const salaryInput = document.querySelector(`#salary_${dealerId}`);
        const turnoverInput = document.querySelector(`#turnover_${dealerId}`);
        const vouchersInput = document.querySelector(`#vouchers_${dealerId}`);
        if (salaryInput)
            salaryInput.value = values.salary;
        if (turnoverInput)
            turnoverInput.value = values.turnover;
        if (vouchersInput)
            vouchersInput.value = values.vouchers;
    });
}
// Вземане на избрания филтър
export function getFilterLocation() {
    const select = document.querySelector(selectors.filterLocation);
    if (!select || select.value === 'all')
        return 'all';
    return parseInt(select.value);
}
// Рендериране на списъка с дилъри (стар метод за съвместимост)
export function renderDealers(dealers, locations, onRemove, filterLocationId = 'all') {
    const container = getElement(selectors.dealersList);
    ...
    // Групиране по обекти
    const dealersByLocation = new Map();
...
export function renderDealersForLocation(dealers, onRemove) {
    const container = getElement(selectors.dealersList);
    // Запазване на текущите стойности преди рендериране
    const savedValues = saveCurrentInputValues(dealers);
    if (dealers.length === 0) {
...
export function getNewDealerDataSimple() {
    const name = getInputValue(selectors.dealerName);
    const coefGeneral = getNumberValue(selectors.coefGeneral);
    const coefPersonal = getNumberValue(selectors.coefPersonal);
    if (!name) {
...
export function getMonthlyData(dealers) {
...
export function getGlobalTurnover() {
...
export function getTarget() {
    const input = document.querySelector('#targetTurnover');
    if (!input)
        return 0;
    return parseFloat(input.value) || 0;
}
// Показване на статус за таргета
export function showTargetStatus(globalTurnover, target) {
...
export function getMonth() {
    const select = getElement(selectors.month);
    return select.value;
}
// Вземане на годината
...
export function setDefaultMonthYear() {
    const now = new Date();
    const monthSelect = document.querySelector(selectors.month);
    const yearInput = document.querySelector(selectors.year);
    if (monthSelect) {
...
export function renderResults(results) {
    const section = getElement(selectors.resultsSection);
    const tbody = getElement(selectors.resultsBody);
...
export function renderResultsSimple(results) {
    const section = getElement(selectors.resultsSection);
    const tbody = getElement(selectors.resultsBody);
    section.style.display = 'block';
    // Показване на месец и година в заглавието
...
export function clearAllMonthlyInputs(dealers) {
    // Изчистване на общия оборот
    const globalInput = document.querySelector(selectors.globalTurnover);
    if (globalInput) {
        globalInput.value = '';
    }
    // Изчистване на таргета
    const targetInput = document.querySelector('#targetTurnover');
    if (targetInput) {
        targetInput.value = '';
    }
