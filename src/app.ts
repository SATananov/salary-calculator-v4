import { Dealer, Location, CalculationResult, ExcelRow } from './types.js';
...
// Деклариране на XLSX (зарежда се от CDN)
...
let isManageLocationsVisible: boolean = false;
...
function handleLocationSelect(e: Event): void {
    ...
    if (locationId) {
        ...
        if (location) {
            ...
            if (locationNameHeader) locationNameHeader.textContent = displayName;
            if (locationNameDealers) locationNameDealers.textContent = displayName;
...
function toggleManageLocations(): void {
    const manageSection = document.getElementById('manageLocationsSection');
...
function handleRemoveLocation(id: number): void {
...
function handleRemoveDealer(id: number): void {
...
function handleExport(): void {
    ...
    const excelData: ExcelRow[] = results.map(r => ({
        'Месец': r.month,
        'Година': r.year,
...
function handleExcelUpload(e: Event): void {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.globalTurnover) {
                    const globalInput = document.getElementById('globalTurnover') as HTMLInputElement;
                    if (globalInput) {
...
function processExcelData(data: any[], locationDealers: Dealer[]): { 
    success: boolean; 
    matched: number; 
    notFound: string[]; 
    ...
    let matched = 0;
    let globalTurnover: number | undefined;
    ...
    const personalKey = keys.find(k => 
        k.toLowerCase().includes('личен') || 
        k.toLowerCase().includes('собствен') ||
        k.toLowerCase().includes('personal')
    );
    
    // Намиране на колоната за общ оборот
    const globalKey = keys.find(k => 
        k.toLowerCase().includes('общ') || 
    ...
    for (const row of data) {
        ...
        const dealer = locationDealers.find(d => 
            d.name.toLowerCase().trim() === name.toLowerCase()
        );
        
        if (dealer) {
            // Попълване на полето за оборот
            const turnoverInput = document.getElementById(`turnover_${dealer.id}`) as HTMLInputElement;
            if (turnoverInput) {
...
function showUploadStatus(message: string, type: 'success' | 'error'): void {
    const statusDiv = document.getElementById('uploadStatus');
    if (statusDiv) {
